...
import { Collection, HttpRequest, RequestHistory, HttpResponse } from '@/schemas'
...
interface CollectionsState {
    ...
    // Request actions
    saveRequest: (collectionId: string, request: Omit<HttpRequest, 'id' | 'createdAt' | 'updatedAt'>) => string
...
const truncateResponseBody = (response: HttpResponse): HttpResponse => {
    ...
    if (response.body && response.body.length > maxBodySize) {
        return {
            ...response,
            body: response.body.slice(0, maxBodySize) + '\n... [truncated for storage]',
        }
    }
    return response
}
        ...
        (set, get) => ({
            ...
            updateRequest: (id: string, updates: Partial<HttpRequest>) => {
                set((state) => {
                    ...
                    if (existing) {
                        newRequests.set(id, { ...existing, ...updates, updatedAt: new Date() })
                    }
